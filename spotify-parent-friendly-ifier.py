...
def find_clean_version(track_name, artist, token):
    search_url = "https://api.spotify.com/v1/search"
    headers = {"Authorization": f"Bearer {token}"}
    params = {"q": f"{track_name} {artist}", "type": "track", "limit": 5}
...
def main():
    parser = argparse.ArgumentParser(description="Spotify Clean Playlist Creator")
    ...
    for track in tracks:
        if track["explicit"]:
            clean_id = find_clean_version(track["name"], track["artists"][0]["name"], token)
            if clean_id:
            ...
            clean_ids.append(track["id"])
    new_playlist_id = create_new_playlist(args.user_id, "Clean Playlist", token)
    if new_playlist_id:
        add_tracks_to_playlist(new_playlist_id, clean_ids, token)
        print(f"Clean playlist created: {new_playlist_id}")
